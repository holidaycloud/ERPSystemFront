...
orderAction.detail = function (req, res) {
    async.auto({
        "getOpenId": function (cb, result) {
            result.error = 0;
            if(req.query.code){
                ...
                cb("error","验证信息异常，请在微信游览器中重新点击进入");
            }
        },
        ...
        "getOrderDetail": ["getToken", function (cb, result) {
            var reqUrl = config.httpReq.host + ":" + config.httpReq.port + "/api/order/detail?id=" + req.params.id + "&token=" + result.token;
...
//                console.log("------------------------->order detail:",obj);
        ...
        if (err) {
            ...
            res.render("wap_order_detail", data);
        }
    });
}
